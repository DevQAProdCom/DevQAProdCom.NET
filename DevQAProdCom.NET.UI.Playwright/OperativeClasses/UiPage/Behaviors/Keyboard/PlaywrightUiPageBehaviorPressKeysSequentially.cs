...
{
    public class PlaywrightUiPageBehaviorPressKeysSequentially(IBehaviorParameters parameters, IKeyMatcher keyMatcher) : PlaywrightUiPageBehavior(parameters), IUiPageBehaviorPressKeysSequentially
    {
        public void PressKeysSequentially(params string[] keys)
        {
            foreach (var key in keys)
            {
                ...
                if (matchData.IsMatched)
                    Page.Keyboard.PressAsync(matchData.MatchedKey).Wait();
                else
                {
                    char[] chars = matchData.Key.ToCharArray();

                    foreach (char @char in chars)
                        Page.Keyboard.PressAsync(@char.ToString()).Wait();
                }
            }
        }
